...
class CollaborationsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CollaborationsScreen> createState() =>
      _CollaborationsScreenState();
}
          ...
          return TabBarView(
            ...
            children: [
              ...
              _CollabList(
                ...
                userId: userId,
                repo: repo,
                emptyMsg: 'No accepted collaborations',
              ),
              _CollabList(
                items: all
                    .where((c) => c.status == CollaborationStatus.inProgress)
                    .toList(),
                userId: userId,
                repo: repo,
                emptyMsg: 'No active collaborations',
              ),
              _CollabList(
                items: all
                ...
                userId: userId,
                repo: repo,
                emptyMsg: 'No completed collaborations yet',
              ),
              _CollabList(
                items: all
                    .where((c) => c.status == CollaborationStatus.cancelled)
                    .toList(),
                userId: userId,
                repo: repo,
                emptyMsg: 'No history',
...
class _CollabList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
        ...
        child: Column(
          ...
          children: [
            ...
            TextButton(
              ...
              child: const Text('Find people to collaborate with'),
            ),
          ],
        ),
      );
    ...
    return ListView.separated(
      ...
      itemBuilder: (_, i) =>
          _CollabCard(collab: items[i], userId: userId, repo: repo),
    );
  }
}
...
class _CollabCard extends StatelessWidget {
  ...
  String get _otherName =>
      _isRequester ? collab.targetName : collab.requesterName;
  String? get _otherPhoto =>
      _isRequester ? collab.targetPhoto : collab.requesterPhoto;
          ...
          if (collab.scheduledAt != null) ...[
            ...
            Row(
              children: [
                ...
                Text(
                  'Scheduled: ${_formatDate(collab.scheduledAt!)}',
                  style: const TextStyle(
                    fontSize: 12,
                    color: AppColors.primary,
  ...
  String _formatDate(DateTime d) =>
      '${d.day}/${d.month}/${d.year} at ${d.hour}:${d.minute.toString().padLeft(2, '0')}';
}
    ...
    return switch (collab.status) {
      CollaborationStatus.pending =>
        !isRequester
            ...
            : Text(
                'Waiting for ${collab.targetName} to respond...',
                style: const TextStyle(
                  fontSize: 12,
                  color: AppColors.textSecondary,
                ),
              ),
      CollaborationStatus.accepted => Row(
        children: [
          Expanded(
            child: ElevatedButton(
              onPressed: () =>
                  repo.updateStatus(collab.id, CollaborationStatus.inProgress),
              style: ElevatedButton.styleFrom(
      ...
      CollaborationStatus.inProgress => Row(
        children: [
          Expanded(
            child: ElevatedButton(
              onPressed: () =>
                  repo.updateStatus(collab.id, CollaborationStatus.completed),
              style: ElevatedButton.styleFrom(
      ...
      CollaborationStatus.cancelled => const Text(
