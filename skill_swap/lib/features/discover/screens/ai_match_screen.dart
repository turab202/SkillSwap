        ...
        data: (matches) => SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Our AI analysed your profile to find your best matches.',
              ...
              if (matches.isEmpty)
                ...
                ...matches
                    .skip(3)
                    .take(4)
                    .map((u) => _CompactMatchCard(user: u, viewer: viewer)),
                const SizedBox(height: 20),
                const _InsightsCard(),
                const SizedBox(height: 16),
...
class _FullMatchCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final sharedSkills = viewer != null
        ? user.skillsOffered
              .where((s) => viewer!.skillsWanted.contains(s))
              .toList()
        : <String>[];
    final theyWant = viewer != null
        ? user.skillsWanted
              .where((s) => viewer!.skillsOffered.contains(s))
              .toList()
        : <String>[];
    final sameLocation =
        viewer?.location != null && user.location == viewer!.location;
    final sameAvail =
        viewer?.availability.isNotEmpty == true &&
        user.availability == viewer!.availability;
            ...
            child: Column(
              children: [
                _MatchDetailRow(
                  ...
                  label: 'They teach',
                  value: sharedSkills.isEmpty
                      ? 'No direct match'
                      : sharedSkills.take(3).join(', '),
                  highlight: sharedSkills.isNotEmpty,
                ),
                const SizedBox(height: 8),
                _MatchDetailRow(
                  icon: Icons.favorite_outline,
                  label: 'They want to learn',
                  value: theyWant.isEmpty
                      ? 'No direct match'
                      : theyWant.take(3).join(', '),
                  highlight: theyWant.isNotEmpty,
                ),
                const SizedBox(height: 8),
                _MatchDetailRow(
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    _buildCollabIdea(user, viewer, sharedSkills, theyWant),
                    style: const TextStyle(
                      fontSize: 12,
                      color: AppColors.textSecondary,
                      height: 1.4,
  ...
  String _buildCollabIdea(
    UserModel user,
    UserModel? viewer,
    List<String> sharedSkills,
    List<String> theyWant,
  ) {
    ...
    if (teach != null && learn != null)
      return '${user.displayName.split(' ').first} could teach you $teach while you teach them $learn — a perfect skill swap!';
    if (teach != null)
      return '${user.displayName.split(' ').first} could teach you $teach. You could offer something from your skill set in return.';
    if (learn != null)
      return 'You could teach ${user.displayName.split(' ').first} $learn and explore what they can offer you.';
